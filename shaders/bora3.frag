...
float dist = distance(scaledFract, center)-w*fract(u_time/2.0 + w/0.472);
   float radius = sin(u_time * 4.280 + scaledIndex.y);
    ...
    if(dist < radius) {
        color = vec4(fract(u_time /1.608 + dist/0.432),0.055/dist,0.688*dist,w+dist/0.212);
